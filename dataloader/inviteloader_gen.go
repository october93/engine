...
type InviteLoader struct {
	...
	cache map[globalid.ID]*model.Invite
...
// LoadThunk returns a function that when called will block waiting for a invite.
...
func (l *InviteLoader) LoadThunk(key globalid.ID) func() (*model.Invite, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Invite, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &inviteBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *InviteLoader) LoadAll(keys []globalid.ID) ([]*model.Invite, []error) {
	results := make([]func() (*model.Invite, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	invites := make([]*model.Invite, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		invites[i], errors[i] = thunk()
	}
...
func (l *InviteLoader) unsafeSet(key globalid.ID, value *model.Invite) {
	if l.cache == nil {
		l.cache = map[globalid.ID]*model.Invite{}
	}
	l.cache[key] = value
}
...
func (b *inviteBatch) keyIndex(l *InviteLoader, key globalid.ID) int {
...
func (b *inviteBatch) startTimer(l *InviteLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *inviteBatch) end(l *InviteLoader) {
