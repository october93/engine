...
func (dg *DocGenerator) generate() error {
	...
	funcs["unfold"] = unfold
	tmpl.Funcs(funcs)
	docTemplate, err := ioutil.ReadFile("../cmd/rpcgen/subcmd/doc.html.tmpl")
	if err != nil {
		return err
	}
	...
	ls, err := ioutil.ReadDir("../cmd/rpcgen/subcmd/assets")
	if err != nil {
		return err
	}
	for _, file := range ls {
		// TODO (konrad) refactor to use something offered by the Go library?
		cmd := exec.Command("cp", fmt.Sprintf("../cmd/rpcgen/subcmd/assets/%s", file.Name()), fmt.Sprintf("../docs/%s", file.Name())) // #nosec
...
func bootstrapExamples(filename string) error {
	...
	t, err := ioutil.ReadFile("../cmd/rpcgen/subcmd/examples.go.tmpl")
	if err != nil {
		return err
	}
	...
	res, err := imports.Process("../cmd/rpcgen/subcmd/examples.go", buf.Bytes(), nil)
	if err != nil {
		return err
	}
	return ioutil.WriteFile("../cmd/rpcgen/subcmd/examples.go", res, 0666)
}
...
func main() {
	...
	dg, err := NewDocGenerator(p, "examples.go")
	if err != nil {
		fail(err)
	}
